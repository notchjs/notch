...
import { HookContainer, HookFactory, HookRecord } from '../../src';
...
describe('HookFactory', () => {
  ...
  it('should prepare hook verbatim when class instance ', () => {
    ...
    expect(factory.prepare(hook)).to.eql({
      name: 'OnStartupHook',
      hook: hook,
    });
  });
  ...
  it('should throw when invalid type', async () => {
    ...
    expect(() => factory.prepare(123)).to.throw();
  });
});
